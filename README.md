...
- Two-Stage Validation Gate (`lib/validation.ts`): Nothing expensive runs until a pitch clears both stages.
  - Stage 1 (free, deterministic): empty input, too-short input and keyboard mashes ("fgbfg") are rejected with zero API cost. A short pitch (under 20 words) with no concrete detail at all - no buyer, market or channel, price or quantity, material or process, or differentiator - is rejected as too vague (`lib/specificity.ts`). Length is not the test, specificity is: "Manufacturing tablets for the health industry at Rs 500" passes, "I want to start a shoe business" is rejected. Stage 1 only makes the calls a keyword check can make with near-certainty; partly-specific pitches go to Stage 2.
  - Stage 2 (one cheap grounded call, sampled twice): judges whether the concept is real at all and written with genuine intent. Catches pitches that read as valid English but are not assessable - a product its stated user cannot physically use ("headphones for fishes"), a goal rather than a product ("I want to be rich"), a low-effort placeholder or hype paragraph with concrete specifics of fewer than two different kinds ("We will sell cotton bedsheets for regular use"), a head-on clone of a dominant incumbent with no stated wedge, or capital off by 10x or more in either direction (a garment unit demanding INR 100 crore, a semiconductor fab on INR 4 lakh).
...
| **What It Catches** | Empty input, length < 15 chars, keyboard mashes (`fgbfg`, `asdfghjkl`), short pitches with no concrete detail at all | Non-physical concepts, generic desires, low-effort placeholders, clone without wedge, CapEx off by >10x | Valid hardware ventures needing deep audit |
