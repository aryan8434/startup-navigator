      ...
      <section className="relative border-b border-slate-800 bg-gradient-to-b from-slate-900 to-slate-950 py-12 md:py-16">
        <div className="max-w-4xl mx-auto px-4 text-center">
          <div className="inline-flex items-center space-x-2 rounded-full bg-purple-500/10 px-4 py-1.5 text-xs font-semibold text-purple-400 border border-purple-500/20 mb-4">
            ...
            <span>Hardware & Manufacturing Economics</span>
          </div>
          ...
          <p className="mt-4 text-base md:text-lg text-slate-300">
            Simulate COGS, tooling payback schedules, gross margins, and monthly break-even unit volumes for your manufacturing startup.
          </p>
        </div>
      </section>
          ...
          <div className="lg:col-span-7 space-y-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              <div className="rounded-2xl border border-slate-800 bg-slate-900/40 p-5 text-center">
                ...
                <p className="text-2xl font-extrabold text-white mt-1">${cogsPerUnit.toFixed(2)}</p>
              ...
              <div className="rounded-2xl border border-emerald-900/50 bg-emerald-950/20 p-5 text-center">
                ...
                <span className="text-[10px] text-emerald-300">${grossProfitPerUnit.toFixed(2)} profit / unit</span>
              </div>
            ...
            <div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-6 backdrop-blur-md space-y-4">
              <h3 className="text-base font-bold text-white flex items-center space-x-2">
                ...
                <span>Monthly Pro-Forma Income Statement</span>
              </h3>
              ...
              <div className="space-y-3 text-sm">
                ...
                <div className="flex justify-between py-2 border-b border-slate-800">
                  ...
                  <span className="font-semibold text-rose-400">-${monthlyCogs.toLocaleString()}</span>
                </div>

                <div className="flex justify-between py-2 border-b border-slate-800">
                  <span className="text-slate-300">Gross Profit</span>
                  <span className="font-semibold text-emerald-400">${monthlyGrossProfit.toLocaleString()}</span>
                </div>

                <div className="flex justify-between py-2 border-b border-slate-800">
                  <span className="text-slate-300">Fixed Monthly Factory Overhead / Operating Expense</span>
                  <span className="font-semibold text-rose-400">-${inputs.monthlyOverhead.toLocaleString()}</span>
                </div>
                ...
                <div className="flex justify-between py-3 px-4 rounded-xl bg-slate-950 text-base font-bold text-emerald-400 border border-emerald-900/30">
                  ...
                  <span>${monthlyOperatingIncome.toLocaleString()}</span>
                </div>
              </div>
            </div>
            ...
            <div className="rounded-2xl border border-slate-800 bg-slate-900/40 p-6 backdrop-blur-md">
              <h3 className="text-base font-bold text-white mb-2">Break-Even & Viability Summary</h3>
              <p className="text-xs text-slate-300 leading-relaxed">
                To cover your monthly factory overhead of <strong>${inputs.monthlyOverhead.toLocaleString()}</strong>, your business must produce and sell at least <strong className="text-indigo-400">{breakEvenUnitsMonth} units</strong> every month. 
                At your projected volume of <strong>{inputs.monthlyVolume} units</strong>, your net monthly operating profit is <strong className="text-emerald-400">${monthlyOperatingIncome.toLocaleString()}</strong>.
              </p>
            </div>
          </div>
        </div>
